...
export async function getAllRoutes(): Promise<RouteInfo[]> {
  try {
    ...
    return response.documents.map((route) => ({
      ...
      endTimestamp: route.endTimestamp ? Number.parseInt(route.endTimestamp) : undefined,
...
export async function getRoutesByBusNumber(busNumber: string): Promise<RouteInfo[]> {
  try {
    const databaseId = config.databaseId
    const collectionId = getRoutesCollectionId()
...
export async function updateRouteStatus(routeId: string, active: boolean): Promise<boolean> {
  try {
    const databaseId = config.databaseId
    const collectionId = getRoutesCollectionId()
    ...
    await databases.updateDocument(databaseId, collectionId, routeId, {
      active: active,
      ...(active ? {} : { endTimestamp: Date.now().toString() }),
    })
